#!/bin/bash
# <xbar.title>Keyboard battery</xbar.title>
...
if [[ $(uname -m) == 'arm64' ]]; then
  PERCENTAGE=$(ioreg -c AppleDeviceManagementHIDEventService -r -l | grep -i KBConnected -A 20  | grep BatteryPercent | cut -d = -f2 | cut -d ' ' -f2)
fi
